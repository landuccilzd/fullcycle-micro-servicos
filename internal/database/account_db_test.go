...
func (s *AccountDBTestSuite) SetupSuite() {
	...
	s.client, _ = entity.NewClient("Zelda", "zelda@hyrule.com")
}
...
func (s *AccountDBTestSuite) TestFindByID() {
	...
	zeldaAccount, err := s.accountDB.FindByID(account.ID)
	s.Nil(err)
	s.NotNil(zeldaAccount)
	s.Equal(account.ID, zeldaAccount.ID)
	s.Equal(account.Balance, zeldaAccount.Balance)
	s.Equal(account.Client.ID, zeldaAccount.Client.ID)
	s.Equal(account.Client.Name, zeldaAccount.Client.Name)
	s.Equal(account.Client.Email, zeldaAccount.Client.Email)
}
